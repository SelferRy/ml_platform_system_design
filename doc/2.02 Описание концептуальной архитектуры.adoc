...
| Централизованное хранилище секретов для всех сервисов: DB credentials, API-ключи, TLS-сертификаты, S3 credentials
| Dynamic secrets (auto-rotation), встроенный audit log, least-privilege access per service, cloud-agnostic

| mTLS (Baseline: TLS, MVP: mTLS)
| Аутентификация и шифрование трафика между внутренними сервисами
| Zero-trust network model; в Baseline K8s NetworkPolicy + TLS достаточен; mTLS (Istio) — extension point MVP при росте числа сервисов
|===
...
| HashiCorp Vault
| Централизованное управление секретами: dynamic credentials для PostgreSQL (TTL 1h), API-ключи, TLS-сертификаты (PKI engine), envelope encryption (Transit engine); Vault Agent Sidecar — инжекция секретов без изменений кода сервисов
| HashiCorp Vault (HA, 3 реплики в K8s)

| OPA (Open Policy Agent)
| Policy-as-code для авторизации: rego-правила в Git, тесты в CI; extension point для MVP — в Baseline RBAC enforcement в API Gateway middleware
| Open Policy Agent (extension point MVP)
|===